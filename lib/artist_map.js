...
async function location_clicked ( loc_id )
{
    ...
    for ( const b of loc_bands )
    {
        ...
        artist_div.innerHTML += 
            `<div class="artist_info">${band_info}</span>`;
    }
}
...
async function wiki_data_html ( id ) 
{
    ...
    let article = data.article.value;
    let name = data.name.value;
    let article_el = `<a href="${article}">${name}</a>`;

    let img = data.img.value;
    let img_el = `<img src="${img}"></img>`;

    return article_el+img_el;
